"""Attach newest processed build to Size Down 1.0, create a review submission with version + subs + group version, submit.
Run: cd landed/.credentials && PYTHONPATH=. python3 ~/workspace/sizedown/store/asc_submit.py [--dry-run]"""
import asc, json, sys, time, os
APP='6809072326'; GROUP=os.environ['SIZEDOWN_GROUP']; SUBS=os.environ['SIZEDOWN_SUBS'].split(','); DRY='--dry-run' in sys.argv
